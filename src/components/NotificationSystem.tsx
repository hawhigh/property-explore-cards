...
const NotificationSystem = () => {
  ...
  return <div className="relative">
      ...
      {/* Notification Panel */}
      {isOpen && <Card className="absolute top-12 right-0 w-80 max-h-96 overflow-hidden shadow-lg z-50">
          ...
          <CardContent className="p-0 max-h-80 overflow-y-auto">
            {notifications.length === 0 ? <div className="p-4 text-center text-gray-500">
                ...
                {notifications.map(notification => <div key={notification.id} className={`p-3 border-b hover:bg-gray-50 cursor-pointer ${!notification.isRead ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''}`} onClick={() => markAsRead(notification.id)}>
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-2 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {notification.action && <Button variant="link" size="sm" className="p-0 h-auto text-xs mt-1" onClick={e => {
                    e.stopPropagation();
                    notification.action?.onClick();
                      ...
                      <Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 hover:opacity-100" onClick={e => {
                e.stopPropagation();
